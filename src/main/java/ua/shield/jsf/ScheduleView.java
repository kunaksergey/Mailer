...
public class ScheduleView implements Serializable {
    ...
    private ExtScheduleEvent event = new ExtScheduleEvent();

    private Set<Task> tasks;

    @ManagedProperty("#{taskJsfController}")
    TaskJsfController taskJsfController;
    ...
    private void checkFillDate(ExtScheduleEvent event) throws WrongFillDateException {
        ...
        if(ldtStart.isBefore(ldtNow)){
            throw new WrongFillDateException("You can't create event yerly now");
        }

        //если у нас нет диапазона, то время начала=времени конца
        if(event.getTypeEvent()==TypeEvent.ONCE){
            event.setEndDate(event.getStartDate());
        }

        if(event.getTypeEvent()==TypeEvent.RANGE){
            /*Корректируем время*/
